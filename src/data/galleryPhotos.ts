...
export const GALLERY_PHOTOS: GalleryPhoto[] = [
  ...
  { id: 'xian', title: '서안 병마용', src: `https://commons.wikimedia.org/wiki/Special:FilePath/${encodeURIComponent('Terracotta Army.jpg')}?width=1400`, catalog: true },
...
export function galleryPhotoById(id?: string | null): GalleryPhoto | undefined {
  ...
  return GALLERY_PHOTOS.find((row) => row.id === id)
}

export function mergeGallery(userPhotos: GalleryPhoto[]): GalleryPhoto[] {
  const extra = userPhotos.filter((row) => !row.catalog && !GALLERY_PHOTOS.some((c) => c.id === row.id))
  return [...GALLERY_PHOTOS, ...extra]
}
